...
router.get('/:id', auth, async(req, res) => {
    ...
    // check the results
    if (result === undefined) res.status(status.NOT_FOUND).send(getResponseForm(null, null, 'The movie with the given ID was not found.'));
...
router.get('/', auth, async(req, res) => {
    ...
    // check the results
    if (results.length === 0) res.status(status.NOT_FOUND).send(getResponseForm(null, null, 'The movie list is empty.'));
...
router.post('/', [auth, admin], async(req, res) => {
    ...
    if (genre === undefined) return res.status(status.BAD_REQUEST).send(getResponseForm(null, null, "Invalid genre"));
...
router.put('/:id', [auth, admin], async(req, res) => {
    ...
    if (!await updateMovie(Movie, req)) res.status(status.NOT_FOUND).send(getResponseForm(null, null, 'The movie with the given ID was not found.'));
...
router.delete('/:id', [auth, admin], async(req, res) => {
    ...
    if (!await deleteMovie(Movie, req)) res.status(status.NOT_FOUND).send(getResponseForm(null, null, 'The movie with the given ID was not found.'));
    else res.send(getResponseForm(null, null, 'Update movie successful.'));
});
